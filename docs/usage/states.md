...
- `Sync` with excluded memory or disk policy is also a no-op, so state stays unchanged
...
- `Sync` also silently does nothing when the current memory or disk state is excluded by `Context.SyncPolicy`
